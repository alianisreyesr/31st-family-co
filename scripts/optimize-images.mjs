...
for (const slug of slugs) {
  ...
  for (const archivo of originales) {
    ...
    metadatos[`/products/${slug}/${base}`] = { width: info.width, height: info.height }
    porCarpeta[slug].push(`/products/${slug}/${base}`)
 ...
 * las medidas del JPEG de respaldo de ${FALLBACK_WIDTH}px, que es lo que
 ...
 */
export const ANCHOS = ${JSON.stringify(WIDTHS).replace(/,/g, ', ')}
export const ANCHO_RESPALDO = ${FALLBACK_WIDTH}

export const productImages = ${JSON.stringify(metadatos, null, 2)}

export const photosByDir = ${JSON.stringify(porCarpeta, null, 2)}
`
